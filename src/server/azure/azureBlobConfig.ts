...
export function assertAzureBlobConfigured(config: AzureBlobConfig = loadAzureBlobConfig()): void {
    if (!config.enabled) {
        throw new Error('Azure Blob storage is disabled on this instance')
    }
    if (!config.storageAccount || !config.container || !config.accountKey) {
        throw new Error('Azure Blob storage is not configured (set x_2058901_fresher.azure.* system properties)')
    }
}
